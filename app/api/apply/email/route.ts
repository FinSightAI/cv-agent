...
export async function POST(req: NextRequest) {
  ...
  if (coverLetter) {
    ...
    const greeting = lang === "he"
      ? `<p>שלום,<br>אני מגיש/ה מועמדות לתפקיד <strong>${jobTitle}</strong> ב-<strong>${company}</strong>.</p>`
      : `<p>Hello,<br>I am applying for the <strong>${jobTitle}</strong> position at <strong>${company}</strong>.</p>`;
    ...
    ? [
        {
          ...
          content: Buffer.from(tailoredCvMd, "utf-8").toString("base64"),
          type: "text/markdown",
        },
      ]
    : [];
